...
<?php
   ...
   $recordss = $conn->prepare("SELECT id,nombre,id_categoria FROM productos WHERE id_categoria=2");
   $recordss->bindParam(':id', $_POST['id']);
   $recordss->execute();
   $resultado = $recordss->fetch(PDO::FETCH_ASSOC);

//    $sqla = $mysqli -> query("SELECT * FROM productos ORDER BY id DESC");
//    $rowp =$sqla->fetch_array();

//    $countLikes = null;

//    $countLikes = $mysqli -> query("SELECT * FROM users where id = '".$_SESSION['id']."' AND producto ='".$rowp['id']."'");
